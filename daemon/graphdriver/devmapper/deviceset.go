...
func (devices *DeviceSet) lookupDeviceLock(hash string) (*devInfo, error) {
	devices.Lock()
...
func (devices *DeviceSet) cleanupDeletedDevices() error {
	...
	// Delete the delete devices. DeleteDevice() first takes the info lock
...
func (devices *DeviceSet) setupBaseImage() error {
	oldInfo, _ := devices.lookupDeviceLock("")
...
func (devices *DeviceSet) AddDevice(hash, baseHash string) error {
	...
	baseInfo, err := devices.lookupDeviceLock(baseHash)
	if err != nil {
		return err
	}
...
func (devices *DeviceSet) DeleteDevice(hash string, syncDelete bool) error {
	...
	info, err := devices.lookupDeviceLock(hash)
	if err != nil {
		return err
	}
...
func (devices *DeviceSet) Shutdown() error {
	...
	info, _ := devices.lookupDeviceLock("")
	if info != nil {
		info.lock.Lock()
		devices.Lock()
		if err := devices.deactivateDevice(info); err != nil {
...
func (devices *DeviceSet) HasDevice(hash string) bool {
	info, _ := devices.lookupDeviceLock(hash)
	return info != nil
}
